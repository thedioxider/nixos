...
let
  nmProfile =
    {
      id, # Name of a network displayed in system
    ...
    let
      ...
      supported_wifi_security = [
        "wpa-personal"
        "wpa-enterprise"
      ];
    in
    assert lib.assertMsg (builtins.elem ptype supported_ptype) ''
      unknown profile type: ${ptype}
      supported: ${builtins.concatStringsSep ", " supported_ptype}
    '';
    assert lib.assertMsg (ptype == "wifi" -> ssid != null) "wifi ssid is required: ${id}";
    assert lib.assertMsg (ptype == "wifi" -> builtins.elem security supported_wifi_security) ''
    ...
    // lib.optionalAttrs (ptype == "wifi") (
      ...
      // lib.optionalAttrs (security == "wpa-enterprise") (
        assert lib.assertMsg (eap-auth != null) "eap-identity is required: ${id}";
  ...
  wpaEnterpriseProfile =
    ...
    let
      supported_eap = [ "peap" ];
    in
    assert lib.assertMsg (builtins.elem eap supported_eap) ''
      unknown EAP: ${eap}
